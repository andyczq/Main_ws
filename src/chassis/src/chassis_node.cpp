...
bool turn_on_tgrobot::GetOdometer_toSensor(Odom_Tgrobot &odom)
{
    ...
    if(serial_buf[13] == Check_CRC(serial_buf, 13))
    {
...
uint8_t turn_on_tgrobot::Check_CRC(uint8_t *data, uint8_t len)
{
    uint8_t crc = 0x00;
    for (int i = 0; i < len; i++) {
        crc ^= data[i];
